...
from torch_geometric.nn import GATv2Conv, GraphNorm, TopKPooling, PANConv
...
class PANConcDQL(torch.nn.Module):
    def __init__(self, num_node_features, num_edge_features, num_actions, seed):
        ...
        self.conv2 = PANConv(self.embedding_size, self.embedding_size, 15)
        self.norm2 = GraphNorm(self.embedding_size)
        self.conv3 = PANConv(self.embedding_size, self.embedding_size, 10)
        self.norm3 = GraphNorm(self.embedding_size)

        
        # Dueling DQN layers
        self.value_stream = torch.nn.Linear(self.embedding_size*3 + num_node_features, 16)
        self.value_2 = torch.nn.Linear(16, 8)
        self.value = torch.nn.Linear(8, 1)

        self.advantage_stream = torch.nn.Linear(self.embedding_size*3 + num_node_features, 64)
