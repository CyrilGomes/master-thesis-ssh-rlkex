...
class PANConcDQL(torch.nn.Module):
    def __init__(self, num_node_features, num_edge_features, num_actions, seed):
        ...
        # Dueling DQN layers
        self.value_stream = torch.nn.Linear(self.embedding_size*2 + num_node_features, 10)
        self.value_2 = torch.nn.Linear(10, 5)
        self.value = torch.nn.Linear(5, 1)

        self.advantage_stream = torch.nn.Linear(self.embedding_size*2 + num_node_features, num_actions*4)
        self.advantage_2 = torch.nn.Linear(num_actions*4, num_actions*2)
