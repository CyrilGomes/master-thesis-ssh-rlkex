...
def test_for_graph(file):
    ...
    for goal in goals:
        ...
        while not done:
            ...
            new_observation, reward, done, info, _ = env.step(action, goal)
...
def execute_for_graph(file, training = True, use_global_exploration = False):
    ...
    for episode in range_episode:
        ...
        experience = namedtuple("Experience", field_names=["observation", "action", "reward", "new_observation", "done", "action_mask", "next_action_mask"])
        experiences = []
        while not done:
            action_mask = env._get_action_mask()
            ...
            new_observation, reward, done, info, new_goal = env.step(action, goal)

            if new_goal is not None:
                goal_one_hot = create_one_hot_goal(new_goal)
                #print(f"New goal : {goal}")
            next_action_mask = env._get_action_mask()
            curr_episode_rewards.append(reward)
            if training:
                experiences.append(experience(observation, action, reward, new_observation, done, action_mask, next_action_mask))
        ...
        for experience in experiences:
            agent.step(experience.observation, experience.action, goal_one_hot, experience.reward, experience.new_observation, experience.done, experience.action_mask, experience.next_action_mask)
