    ...
    def step(self, action):
        ...
        if action == self.action_space.n - 1:
            ...
            if len(self.visited_keys) == self.nb_targets and  self.nb_targets == nb_true_keys:
                reward = 100 * len(self.visited_keys)
                done = True
                found_target = True
                print("DAMN")
            else:
                reward = -8 * len(self.visited_keys) + nb_true_keys
                done = True
                found_target = False
        ...
        if self.target_node is None:
            ...
            return self._get_obs(), -20, False, {'found_target': False,
