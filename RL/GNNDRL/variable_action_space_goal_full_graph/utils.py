...
def add_global_root_node(graph):
    root_node = "root"
    graph.add_node(root_node, label=root_node, cat=-1, struct_size=0, pointer_count=0, valid_pointer_count=0, invalid_pointer_count=0, first_pointer_offset=0, last_pointer_offset=0, first_valid_pointer_offset=0, last_valid_pointer_offset=0)
    #add an edge between the root node and one of the node of each connected component
    for component in nx.connected_components(graph):
        graph.add_edge(root_node, random.choice(list(component)), offset=0)
