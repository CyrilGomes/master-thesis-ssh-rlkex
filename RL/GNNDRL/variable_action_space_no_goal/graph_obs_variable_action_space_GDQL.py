...
def test_for_graph(file):
    ...
    while not done:
        ...
        action_space = env._get_valid_actions()
        visited_subgraph = env.get_visited_subgraph()
        action = agent.act(observation, action_space, goal, visited_subgraph)
...
def supervised_training(file):
    ...
    num_episodes = 10 * num_episode_multiplier
    ...
    for episode in range_episode:
        ...
        while not done:
            ...
            new_observation, reward, done, info = env.step(action)
            next_action_space = env._get_valid_actions()

            agent.step(observation, action, reward, new_observation, done, next_action_space, goal, visited_subgraph)
...
def execute_for_graph(file, training = True):
    ...
    for episode in range_episode:
        ...
        while not done:
            ...
            action_space = env._get_valid_actions()
            visited_subgraph = env.get_visited_subgraph()
            action = agent.act(observation, action_space, goal,visited_subgraph, curr_eps)
            new_observation, reward, done, info = env.step(action)
            next_action_space = env._get_valid_actions()
            curr_episode_rewards.append(reward)
            if reward > 0:
                print(f"Found key with reward {reward}")
            metrics = {
                'Reward': reward,
                'Epsilon': curr_eps,
